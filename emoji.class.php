...
class Emoji {
	/**
	 * Lista degli emoji in base al nome
	 * @access private
	 * @var array
	 */
	private $map = array();
	...
	public function nameToHtml($name) {
		...
		if (is_string($name)) {
			$name = trim($name, ':');
			if (trim($name) != '' && isset($this->map[$name])) {
				$html = '';
				$hex = explode('-', $this->map[$name]);
				$counter = count($hex);
				for ($i = 0; $i < $counter; $i++) {
					$html .= '&#'.hexdec($hex[$i]).';';
				}
